...
def make_sentence(output):
	...
	if cfg['learn_from_cw']:
		toots = c.execute("SELECT content, cw FROM `toots` ORDER BY RANDOM() LIMIT 10000").fetchall()
	else: 
		toots = c.execute("SELECT content, cw FROM `toots` WHERE cw = 1 ORDER BY RANDOM() LIMIT 10000").fetchall()
	toots_str = ""
	for toot in toots:
		print(toot[1])
		toots_str += "\n{}".format(toot[0])
